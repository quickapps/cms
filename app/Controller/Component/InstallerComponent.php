 ...
 */
    public function install($data = false, $options = array()) {
        ...
        if (($v_result_list = $PclZip->extract(PCLZIP_OPT_PATH, $workingDir . 'unzip')) == 0 ) {
            ...
            /** DB Logics **/
            $moduleData = array(
                'name' => ($this->options['type'] == 'module' ? $appName : 'theme_' . Inflector::underscore($appName)),
 ...
 */
    public function uninstall($pluginName = false) {
        ...
        $name = Inflector::underscore($this->options['name']);
 ...
 * }}}
 *
 * @param String $plugin underscored plugin name to analyze
 ...
 */
    public function buildAcos($plugin, $pluginPath = false) {
        ...
        $appControllerPath = $cPath . Inflector::camelize($plugin) . 'AppController.php';
