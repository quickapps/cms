<?php echo $this->Form->create('User', array('url' => "/user/register/")); ?>
    ...
    <?php echo $this->Html->useTag('fieldsetstart', __t('Create new account')); ?>
        <?php echo $this->Form->input('name', array('type' => 'text', 'label' => __t('Real name *'))); ?>
        <em><?php echo __t('Your real name, it is used only for identification purposes. i.e: John Locke'); ?></em>
        <?php echo $this->Form->input('username', array('type' => 'text', 'label' => __t('User name *'))); ?>
        ...
        <?php echo $this->Form->input('email', array('type' => 'text', 'label' => __t('E-mail'))); ?>
        ...
        <?php foreach ($fields as $field): ?>
            <?php echo $this->Layout->hook(Inflector::underscore($field['field_module']) . "_edit", $field, array('collectReturn' => false)); ?>
        <?php endforeach; ?>
