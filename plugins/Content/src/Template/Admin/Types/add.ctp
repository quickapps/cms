    ...
    <div class="col-md-12">
        <?= $this->Form->create($type); ?>
            <fieldset>
                ...
                <?= $this->Form->input('slug', ['label' => __d('content', 'Machine name *')]); ?>
                <em class="help-block">
                    <?= __d('content', 'A unique name for this content type, accepted characters are lowercase letters, numbers and "-" symbol (a-z, 0-9, -). Leave empty to automatically generate one from content name. This value can not be changed after content type is created.'); ?>
                </em>
            ...
            <?= $this->Form->submit(__d('content', 'Create and continue »')); ?>
        <?= $this->Form->end(); ?>
    </div>
</div>