...
<?= $this->Form->create($user, ['id' => 'user-form']); ?>
    <?= $this->Flash->render('user_profile'); ?>
    <fieldset>
        ...
        <?= $this->Form->input('public_email', ['type' => 'checkbox', 'label' => __d('user', 'Public e-mail')]); ?>
        <em class="help-block"><?= __d('user', 'Other users can see your email in <a href="{0}">your profile page</a>.', $this->Url->build(['plugin' => 'User', 'controller' => 'gateway', 'action' => 'profile', $user->id])); ?></em>

        <?= $this->Form->input('public_profile', ['type' => 'checkbox', 'label' => __d('user', 'Public profile')]); ?>
        <em class="help-block"><?= __d('user', 'Restrict the access to <a href="{0}">your profile page</a>.', $this->Url->build(['plugin' => 'User', 'controller' => 'gateway', 'action' => 'profile', $user->id])); ?></em>
    ...
    <?= $this->Html->link(__d('user', 'Cancel my account'), ['plugin' => 'User', 'controller' => 'gateway', 'action' => 'cancel_request']); ?>
<?= $this->Form->end(); ?>