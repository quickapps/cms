 ...
 *
 *     // scope-tag name: author
 *     // scope-tag arguments: >2014-03-01
 *     date:>2014-03-01
 *
 * You can create custom scope-tags using the `SearchableBehavior::addScopeTag()` method.
 * For example, you may want to create a custom criteria `author` which allows
	...
	public function scopeQuery($criteria, $query = null) {
		...
		foreach ($tokens as $k => $t) {
			...
			if (preg_match('/(-?)(\w+)\:(.+)$/', $t, $custom)) {
				...
				if ($orAnd === 'or') {
					$query->orWhere(["SearchDatasets.words {$LIKE}"  => "%{$t}%"]);
				} elseif ($orAnd === 'and') {
					$query->andWhere(["SearchDatasets.words {$LIKE}" => "%{$t}%"]);
				} else {
					$query->where(["SearchDatasets.words {$LIKE}"  => "%{$t}%"]);
				}
			}
		}
